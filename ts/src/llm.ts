...
The CAS checks axioms by **normalizing both sides using axioms as left-to-right rewrite rules, then comparing normal forms**. It uses size-based axiom orientation and bounded bidirectional rewriting as a fallback.

Key consequences:

1. **Axioms generally cannot be omitted even if logically derivable.** The CAS uses term-rewriting, not full equational reasoning. It cannot derive \`right_unit\` from \`assoc + left_unit + left_inv\` even though this is a standard theorem. If removing an axiom causes a verification failure, add it back.
