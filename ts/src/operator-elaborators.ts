...
function generateRealizability(theory: TheoryJson): string {
  ...
  lines.push("-- χ(x)(a) = True (every element realizes the characteristic predicate).");
  lines.push("-- This is the \"constant true\" assembly morphism X → Ω.");
  lines.push("--");
  lines.push("-- The real characteristic morphism χ_m for a specific mono m needs");
  lines.push("-- χ_m(x)(a) = ∃ s, m(s) = x ∧ S.realizes a s, but making this");
  lines.push("-- into an AssemblyHom requires a PCA element that *searches* for");
  lines.push("-- preimages — this is the computational content of the subobject");
  lines.push("-- classifier and requires the PCA to be \"rich enough\" (e.g., Kleene's");
  lines.push("-- first algebra K₁). We define the simpler version here and prove");
  lines.push("-- the pullback property for the image-restricted predicate separately.");
  lines.push("def charMorphismTrue {A : Type u} [PCA A]");
  lines.push("    (X : Assembly A) :");
  lines.push("    AssemblyHom A X (omegaAssembly A) where");
  lines.push("  func := fun _ => trueProp A");
  lines.push("  tracker := PCA.k");
  lines.push("  tracked := by");
  lines.push("    intro x a hax");
  lines.push("    obtain ⟨ka, hka⟩ := PCA.k_app₁ (A := A) a");
  lines.push("    exact ⟨ka, hka, trivial⟩");
  lines.push("");
  lines.push("-- The image-restricted predicate: for a mono m, define the predicate");
  lines.push("-- that is \"true at x\" iff x is in the image of m.");
  lines.push("-- This is the *func* component of the real characteristic morphism.");
  lines.push("noncomputable def charPred {A : Type u} [PCA A]");
  lines.push("    (S X : Assembly A) (m : AssemblyHom A S X)");
  lines.push("    (x : X.carrier) : RealizedProp A where");
  ...
  lines.push("-- The pullback property: for any s : S, the predicate charPred at m(s)");
  lines.push("-- is realized by every realizer of s.");
  lines.push("theorem subobject_classifier_pullback {A : Type u} [PCA A]");
  lines.push("    (S X : Assembly A) (m : AssemblyHom A S X)");
  lines.push("    (s : S.carrier) (a : A) (ha : S.realizes a s) :");
  lines.push("    (charPred S X m (m.func s)).pred a := by");
  lines.push("  -- a realizes s, and m(s) = m(s), so (∃ s', m(s') = m(s) ∧ S.realizes a s')");
  lines.push("  exact Or.inl ⟨s, rfl, ha⟩");
  lines.push("");
  lines.push("-- Converse: if charPred at x is realized by a (via the left disjunct),");
  lines.push("-- then x is in the image of m.");
  lines.push("theorem subobject_classifier_converse {A : Type u} [PCA A]");
  lines.push("    (S X : Assembly A) (m : AssemblyHom A S X)");
  lines.push("    (x : X.carrier) (a : A)");
  lines.push("    (h : ∃ s, m.func s = x ∧ S.realizes a s) :");
  lines.push("    ∃ s, m.func s = x := by");
  lines.push("  obtain ⟨s, hs, _⟩ := h");
  lines.push("  exact ⟨s, hs⟩");
  lines.push("");
  ...
  lines.push("      -- Step 1: f ~ f' on dom(R): f·a and f'·a are S-related");
  lines.push("      obtain ⟨b₁, b₂, hfb₁, hf'b₂, hb₁b₂⟩ := hff' a ha");
  lines.push("      -- Step 2: g.respect(b₁, b₂): g·b₁ and g·b₂ are T-related");
  lines.push("      obtain ⟨c₁, c₂, hgc₁, hgc₂, hc₁c₂⟩ := g.respect b₁ b₂ hb₁b₂");
  lines.push("      -- Step 3: b₂ ∈ dom(S) because b₁ S b₂ → b₂ S b₁ → b₂ S b₂");
  lines.push("      have hb₂dom : S.dom b₂ := S.trans b₂ b₁ b₂ (S.symm b₁ b₂ hb₁b₂) hb₁b₂");
  lines.push("      -- Step 4: g ~ g' at b₂: g·b₂ and g'·b₂ are T-related");
  lines.push("      obtain ⟨d₁, d₂, hgd₁, hg'd₂, hd₁d₂⟩ := hgg' b₂ hb₂dom");
  lines.push("      -- Step 5: g·b₂ = c₂ = d₁ (deterministic application)");
  lines.push("      have hc₂_eq_d₁ : c₂ = d₁ := Option.some.inj (hgc₂ ▸ hgd₁)");
  lines.push("      subst hc₂_eq_d₁");
  lines.push("      -- Step 6: chain: c₁ T c₂ T d₂, so c₁ T d₂");
  lines.push("      have hc₁d₂ : T.rel c₁ d₂ := T.trans c₁ c₂ d₂ hc₁c₂ hd₁d₂");
  lines.push("      -- Step 7: comp(f,g)·a = c₁ and comp(f',g')·a = d₂");
  lines.push("      exact ⟨c₁, d₂,");
  lines.push("        PCA.comp_tracker_app f.tracker g.tracker a b₁ c₁ hfb₁ hgc₁,");
  lines.push("        PCA.comp_tracker_app f'.tracker g'.tracker a b₂ d₂ hf'b₂ hg'd₂,");
  lines.push("        hc₁d₂⟩)");
  ...
  lines.push("      intro a ha");
  lines.push("      -- Both (h ∘ g) ∘ f and h ∘ (g ∘ f) compute h·(g·(f·a)).");
  lines.push("      -- We show they produce U-equivalent outputs.");
  lines.push("      --");
  lines.push("      -- LHS tracker: comp(comp(fd, gd), hd)·a");
  lines.push("      --   = hd · (comp(fd, gd) · a)");
  lines.push("      --   = hd · (gd · (fd · a))");
  lines.push("      --");
  lines.push("      -- RHS tracker: comp(fd, comp(gd, hd))·a");
  lines.push("      --   = comp(gd, hd) · (fd · a)");
  lines.push("      --   = hd · (gd · (fd · a))");
  lines.push("      --");
  lines.push("      -- Same PCA output! So U-equivalence is reflexive.");
  lines.push("      --");
  lines.push("      -- Step 1: f·a = b (using fd.respect a a ha)");
  lines.push("      obtain ⟨b, b', hfb, hfb', hbb'⟩ := fd.respect a a ha");
  lines.push("      -- Step 2: g·b = c (using gd.respect, need b ∈ dom(S))");
  lines.push("      have hbdom : S.dom b := S.trans b b' b hbb' (S.symm b b' hbb')");
  lines.push("      obtain ⟨c, c', hgc, hgc', hcc'⟩ := gd.respect b b hbdom");
  lines.push("      -- Step 3: h·c = d (using hd.respect, need c ∈ dom(T))");
  lines.push("      have hcdom : T.dom c := T.trans c c' c hcc' (T.symm c c' hcc')");
  lines.push("      obtain ⟨d, d', hhd, hhd', hdd'⟩ := hd.respect c c hcdom");
  lines.push("      -- Step 4: LHS = comp(comp(fd,gd), hd)·a");
  lines.push("      -- comp(fd,gd)·a = gd·(fd·a) = gd·b = c");
  lines.push("      have h_fg_a : PCA.app (PCA.comp_tracker fd.tracker gd.tracker) a = some c :=");
  lines.push("        PCA.comp_tracker_app fd.tracker gd.tracker a b c hfb hgc");
  lines.push("      -- comp(comp(fd,gd), hd)·a = hd·c = d");
  lines.push("      have h_lhs : PCA.app (PCA.comp_tracker (PCA.comp_tracker fd.tracker gd.tracker) hd.tracker) a = some d :=");
  lines.push("        PCA.comp_tracker_app (PCA.comp_tracker fd.tracker gd.tracker) hd.tracker a c d h_fg_a hhd");
  lines.push("      -- Step 5: RHS = comp(fd, comp(gd,hd))·a");
  lines.push("      -- comp(gd,hd)·b = hd·(gd·b) = hd·c = d");
  lines.push("      have h_gh_b : PCA.app (PCA.comp_tracker gd.tracker hd.tracker) b = some d :=");
  lines.push("        PCA.comp_tracker_app gd.tracker hd.tracker b c d hgc hhd");
  lines.push("      -- comp(fd, comp(gd,hd))·a = comp(gd,hd)·(fd·a) = comp(gd,hd)·b = d");
  lines.push("      have h_rhs : PCA.app (PCA.comp_tracker fd.tracker (PCA.comp_tracker gd.tracker hd.tracker)) a = some d :=");
  lines.push("        PCA.comp_tracker_app fd.tracker (PCA.comp_tracker gd.tracker hd.tracker) a b d hfb h_gh_b");
  lines.push("      -- Step 6: Both sides give d, so d U d by reflexivity");
  lines.push("      exact ⟨d, d, h_lhs, h_rhs, U.trans d d' d hdd' (U.symm d d' hdd')⟩))");
  lines.push("");
