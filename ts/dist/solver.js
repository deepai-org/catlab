...
exports.InverseProblemSolver = void 0;
class InverseProblemSolver {
    constructor(catlab, llm) {
        this.catlab = catlab;
        this.llm = llm;
    }
    /**
     ...
     */
    async solve(targetName, options) {
        const maxRounds = options.maxRounds ?? 5;
        const timeoutMs = options.leanTimeoutMs ?? 30000;
        const history = [];
        ...
        const summaryRes = await this.catlab.requestOrThrow({ command: "summary", theory: targetName }, timeoutMs);
        ...
        if (options.stylePrompt)
            console.error(`[solver] Style: ${options.stylePrompt}`);
        let candidate;
        let lastResult;
        for (let round = 1; round <= maxRounds; round++) {
            ...
            try {
                if (round === 1 || !candidate || !lastResult) {
                    candidate = await this.llm.generateInitial(targetName, targetJson, options.forwardOp, options.stylePrompt);
                }
                else {
                    candidate = await this.llm.refineWithDiff(targetName, lastResult, candidate, options.forwardOp, round);
                }
            }
            catch (err) {
...
exports.InverseProblemSolver = InverseProblemSolver;
