...
def theoryMetas : List TheoryMeta := [
  ...
  { name := "CohesiveHoTT", symbol := "Coh", group := "foundations",
    description := "Homotopy Type Theory extended with modalities for cohesion. The internal language of cohesive ∞-topoi." },
...
def operatorMetas : List OperatorMeta := [
  ...
  { name := "monad", displayName := "Monadicity", symbol := "Mnd", group := "constructor",
    description := "Extract the monad/comonad associated to an adjunction.",
  ...
  { name := "artin_gluing", displayName := "ArtinGluing", symbol := "AGlu", group := "enrichment",
    ...
    arity := 2 },
  { name := "isbell", displayName := "Isbell", symbol := "Isb", group := "duality",
    description := "Isbell conjugation: the fundamental duality between presheaves and copresheaves.",
    params := [] },
  ...
  { name := "cwf", displayName := "CwF", symbol := "CwF", group := "completion",
