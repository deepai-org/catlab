...
def eilenbergMoore (m : MonadData) : Theory :=
  let algebras := m.base.objects.map fun a =>
    ...
    { id := { name := .root s!"Alg({a.id.name})", index := 0 }
      description := s!"Free T-algebra on {a.id.name}" }

  let structureMaps := m.base.objects.map fun a =>
    let ta := m.functor (.atom a.id)
    { id := { name := .root s!"α_{a.id.name}", index := 0 }
      domain := ta
      codomain := .atom a.id
      description := s!"Structure map for T-algebra on {a.id.name}" }

  let unitLaws := m.base.objects.map fun a =>
    { id := { name := .root s!"unit_law_{a.id.name}", index := 0 }
      leftPath := .comp (.atom m.unit) (.atom { name := .root s!"α_{a.id.name}", index := 0 })
      rightPath := .id (.atom a.id)
      description := s!"Unit law: α ∘ η = id for {a.id.name}" }

  let assocLaws := m.base.objects.map fun a =>
    { id := { name := .root s!"assoc_law_{a.id.name}", index := 0 }
      leftPath := .comp (.atom m.mult) (.atom { name := .root s!"α_{a.id.name}", index := 0 })
      rightPath := .comp (m.functor (.atom { name := .root s!"α_{a.id.name}", index := 0 })) (.atom { name := .root s!"α_{a.id.name}", index := 0 })
